...
def datas(x):  # Return
    ...
    # Construct url from requested city and api key
...
def forecast_data(data):  # extract air quality forecast
    dates_list = []
    # list of dicts containing data for requested day starting on two day before current day
    dates = (data['data']['forecast']['daily']['pm25'])[::]
    for i in dates:
...
def graph(dates_list):
    ...
    df = pd.DataFrame(
        values, columns=["max", "avg", "min", "trend"], index=day_vals)
...
if cont_button and city:
    try:
        ...
        if data["data"] != "Unknown station":
            ...
            if aqi(data) != "-":
                ...
                with st.expander("level evaluation criteria"):
                    st.write(
                        '''Levels of the AQI values are determined with the following table.''')
        ...
        # If the input city doesn't exist
            ...
            st.write(
                ":red[Data is unavlaiable for this city or this city doesn't exist]")
